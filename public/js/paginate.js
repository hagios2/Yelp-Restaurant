var numberOfItems = $('#speakers .container-fluid').length; // Get total number of the items that should be paginated
var limitPerPage = 2; // Limit of items per each page
$('#speakers .container-fluid:gt(' + (limitPerPage - 1) + ')').hide(); // Hide all items over page limits (e.g., 5th item, 6th item, etc.)
...
$(".pagination li.current-page").on("click", function() {
  ...
  if ($(this).hasClass('active')) {
    ...
    $("#speakers .container-fluid").hide(); // Hide all items in loop, this case, all the list groups
    var grandTotal = limitPerPage * currentPage; // Get the total number of items up to the page number that was clicked on

    // Loop through total items, selecting a new set of items based on page number
    for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
      $("#speakers .container-fluid:eq(" + i + ")").show(); // Show items from the new page that was selected
    }
  }
...
$("#next-page").on("click", function() {
  ...
  if (currentPage === totalPages) {
    ...
    $("#speakers .container-fluid").hide(); // Hide all items in the pagination loop
